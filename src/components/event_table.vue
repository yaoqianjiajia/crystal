...
<script>
    export default {
        ...
        computed:{
            notapad(){
                var self = this;
                return self.$store.getters.getEventList.filter(function(d){   //过滤
                    if(self.screen_type !== 0 && self.screen_title === ''){  //有type无title
                        if( d.type === self.screen_type ){
                            return d;
                        }
                    }else if(self.screen_type !== 0 && self.screen_title !== ''){   //有type有title
                        if( d.type === self.screen_type && d.content.indexOf(self.screen_title) !== -1){  //-1表示没找到
                            return d;
                        }
                    }else if(self.screen_type === 0 && self.screen_title !== ''){   //无type有title
